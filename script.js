...
$(document).ready(function () {
  $("#contactForm").submit(function (event) {
    ...
    $.ajax({
      ...
      error: function () {
        $(".response-message").html("Error sending message. Please try again.");
      },
    });
  });
});
...
let pythonStartValue = 0,
    pythonEndValue = 70, // Adjust as needed
    pythonSpeed = 30;
...
let progressPython = setInterval(() => {
    ...
    pythonProgress.style.background = `conic-gradient(#your-python-color ${
        pythonStartValue * 3.6
    }deg, #ededed 0deg)`; /* Replace with your desired color */
